...
jfarm = {
	...
	definePlayerObj: function(playerData) {
		...
		var arm = new sheetengine.Sheet(jfarm.playerWeaponData.centerp, jfarm.playerWeaponData.rot, {
			w: 25,
			h: 25
		});
	...
	correctPointBeforeSend: function(point) {
		var relCenter = {
			x: sheetengine.scene.tilewidth * sheetengine.scene.yardcenterstart.yardx,
			y: sheetengine.scene.tilewidth * sheetengine.scene.yardcenterstart.yardy,
			z: 0
		};
	...
	moveBypass: function(obj, angle, dist) {
		...
		obj.target = {
			x: c.x + (Math.sin(angle)) * dist,
			y: c.y + (Math.cos(angle) * dist),
			z: 0
		};
		obj.steps = 0;
	},
	...
	characterArrived: function(obj) {
		...
		if (jfarm.clickedObj && jfarm.clickedObj.type == "crop")
			jfarm.player.arrivedAtCrop = true;
	...
	moveCamera: function() {
		...
		var center = {
			u: sheetengine.scene.center.u,
			v: sheetengine.scene.center.v
		};
		var player = sheetengine.transforms.transformPoint({
			x: jfarm.player.centerp.x,
			y: jfarm.player.centerp.y,
			z: 0
		});
	...
	harvest: function(obj) {
		...
		if (obj.objectType == 'crop') {
			// we calculate quantity 
			jfarm.getTileDetails(jfarm.currentCropYard, function(response) {
				if (response.success) {
					var tile = response.yard,
						yardHealth = (parseInt(tile.fertility, 10) + parseInt(tile.humidity, 10)) / 2,
						cropProductivity = obj.template.productivity
						quantity = Math.round((yardHealth * parseInt(cropProductivity)) /100);
	...
	redraw: function(full) {
		...
		if (jfarm.drawnObj && jfarm.locationCorners) {
			...
			if (jfarm.hoveredBaseSheet && !jfarm.hoveredObj) {
				if (jfarm.conquer)
					jfarm.highlightHoveredBaseSheet("#52A8EC", "tile");
				else
	...
	getHoveredObjectImproved: function(puv) {
		for (var i = 0; i < sheetengine.objects.length; i++) {
			var obj = sheetengine.objects[i],
				ouv = sheetengine.drawing.getPointuv(obj.centerp),
				canvasSize = obj.canvasSize;
			...
			if (puv.u > ouv.u - canvasSize.relu &&
				...
				for (var j = 0; j < sheets.length; j++) {
					var sheet = sheets[j],
						w = sheet.width,
						h = sheet.height;
					var corner0 = sheet.data.cornersuv[0],
						corner1 = sheet.data.cornersuv[1],
						corner2 = sheet.data.cornersuv[2],
						corner3 = sheet.data.cornersuv[3];

					if (puv.v >= corner0.v && puv.v <= corner2.v && puv.u >= corner3.u && puv.u <= corner1.u) {
						return obj;
					}
					// if(puv.u > ouv.u - canvasSize.relu &&
					// 	puv.u < ouv.u - canvasSize.relu + canvasSize.w &&
					// 	puv.v > ouv.v - canvasSize.relv &&
					// 	puv.v < ouv.v - canvasSize.relv + canvasSize.h){
					// 	// jfarm.lastHoveredObj = obj || jfarm.lastHoveredObj;
					// 	return obj;
					// }
				};
				// return obj;
	...
	highlightHoveredBaseSheet: function(strokeStyle, shape) {
	...
	getBaseSheetFromYard: function(yard) {
		for (var i = 0; i < sheetengine.basesheets.length; i++) {
			...
			if (bs.yardx == yard.yardx && bs.yardy == yard.yardy) {
				return bs
			}
		};
	},
	...
	getDrawingCenterp: function(centerp) { // TODO gérer l'orientation
		switch (jfarm.drawnObj.name.toLowerCase()) {
			case "barn": // barn
				return {
					x: centerp.x + (jfarm.tileWidth * 3) / 2,
					y: centerp.y,
					z: null
				};
				break;
			case "cold storage": // cold storage
				return {
					x: centerp.x + jfarm.tileWidth,
					y: centerp.y - jfarm.tileWidth / 2,
					z: null
				};
				break;
			default:
	...
	getColdStorageLocationCornersuv: function(basesheet) { // TODO : Check boundaries
		var corners = basesheet.corners,
			...
			westNorthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x,
				y: centerp.y - jfarm.tileWidth
			}),
			eastNorthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth * 2,
				y: centerp.y - jfarm.tileWidth
			}),
			eastSouthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth * 2,
				y: centerp.y
			});
	...
	getColdStorageLocationBaseSheets: function(basesheet) { // TODO : Check boundaries
		var centerp = basesheet.centerp,
			...
			centerNorthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth,
				y: centerp.y - jfarm.tileWidth
			}),
			eastNorthBaseSheet = jfarm.getBasesheetByCenterp({
			...
			centerSouthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth,
				y: centerp.y
			});
	...
	getBarnLocationBaseSheets: function(basesheet) {
		var centerp = basesheet.centerp,
			secondBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth,
				y: centerp.y
			}),
			thirdBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth * 2,
				y: centerp.y
			}),
			fourthBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth * 3,
				y: centerp.y
			});
	...
	getBarnLocationCornersuv: function(basesheet) { // TODO : Check boundaries
		var corners = basesheet.corners,
			centerp = basesheet.centerp,
			lastBaseSheet = jfarm.getBasesheetByCenterp({
				x: centerp.x + jfarm.tileWidth * 3,
				y: centerp.y
			});
	...
	drawSelectedObj: function(ajaxResponse){
		if(ajaxResponse.success){
			...
			jfarm.currentLocationBaseSheets.forEach(function(elem) {
				elem.free = false;
			});
	...
	defineColdStorage: function(centerp) {
		...
		var cs = new sheetengine.SheetObject(
			centerp, jfarm.rotSouthNorth, [roofSheet, northSheet, southSheet, westSheet, eastSheet], {
			w: 330,
			h: 250,
			relu: 160,
			relv: 160
	...
	defineBarn: function(centerp) {
		...
		var roofSouthSheet = new sheetengine.Sheet({
			x: 0,
			y: bHeight / 2 - 12,
			z: bHeight + 14
		}, {
		...
		var roofNorthSheet = new sheetengine.Sheet({
			x: 0,
			y: -bHeight / 2 + 12,
			z: bHeight + 14
		}, {
	...
	defineSilo: function(centerp) {
		...
		// Tri Top
		var tri1 = new sheetengine.Sheet({
			x: 0,
			y: 20,
			z: 40
		}, {
		...
		// Tri Est
		var tri2 = new sheetengine.Sheet({
			x: 20,
			y: 0,
			z: 40
		}, {
		...
		// Pillier left
		var pillierLeftSud = new sheetengine.Sheet({
			x: -37,
			y: 40,
			z: 40
		}, {
		...
		var pillierLeftEst = new sheetengine.Sheet({
			x: -40,
			y: 37,
			z: 40
		}, {
		...
		// Pillier middle
		var pillierBottomSud = new sheetengine.Sheet({
			x: 37,
			y: 40,
			z: 40
		}, {
		...
		var pillierBottomEst = new sheetengine.Sheet({
			x: 40,
			y: 37,
			z: 40
		}, {
		...
		// Pillier right
		var pillierRightSud = new sheetengine.Sheet({
			x: 37,
			y: -40,
			z: 40
		}, {
		...
		var pillierRightEst = new sheetengine.Sheet({
			x: 40,
			y: -37,
			z: 40
		}, {
	...
	defineTomatoesPlants: function(centerp, dimmed) {
		...
		for (var i = 0; i < coords.length; i++) {
			var plant = new sheetengine.Sheet(coords[i], {
				alphaD: 0,
				betaD: 0,
				gammaD: 90
			}, {
	...
	defineCrops: function(centerp, name, color, gradient, gradientColor) {
		...
		for (var i = 0; i < nbRows; i++) {
			var rowCenter = {
				...
				y: y,
				z: 4
			}, rowSheet = new sheetengine.Sheet(rowCenter, {
					alphaD: 0,
					betaD: 0,
					gammaD: 0
				}, {
	...
	click: function(event) {
		...
		// ui
		$('#modal-c, #modal-b').hide();
		jfarm.currentCrop = null;
		...
		if (!jfarm.hoveredObj) {
			...
			if (jfarm.clickedObj.type == 'crop') {
				jfarm.currentCrop = jfarm.clickedObj;
				jfarm.currentCropYard = jfarm.clickedYard;
				jfarm.setTarget(jfarm.player, jfarm.clickedObj.centerp);
			} else {
				$('.game').trigger('onBuildingClick', [jfarm.clickedObj]);
			}
		}
	},
	mousemove: function(event) {
		...
		// jfarm.hoveredObj = jfarm.getHoveredObjectImproved(puv);
	...
	timer: function() {
		...
		if (jfarm.player) {
			...
			if (jfarm.player.arrivedAtCrop) {
				jfarm.player.arrivedAtCrop = false;
				$('.game').trigger('onCharacterArrivedAtCrop', [jfarm.currentCrop]);
			}
		}
